...
async function markdownRecords(root: string, relativeDirectory: string): Promise<MarkdownRecord[]> {
  ...
  return records.sort((left, right) => right.relativePath.localeCompare(left.relativePath));
}
  ...
  const employees = await Promise.all(
    entries
      ...
      .map(async (entry) => {
        ...
        try {
          const [content, metadata] = await Promise.all([readFile(absolutePath, "utf8"), stat(absolutePath)]);
          return { id: entry.name, name: firstHeading(content), relativePath, updatedAt: metadata.mtime.toISOString() };
        } catch (error: unknown) {
...
export async function generateDashboardSnapshot(root: string, now: Date): Promise<DashboardSnapshot> {
  const [employees, briefs, risks, reviews, decisions] = await Promise.all([
  ...
  const employeeSnapshots = employees.map((employee) => {
    ...
    return {
      ...
      handoff: "依角色手冊交接",
      blocker: null,
      nextStep: task?.nextStep ?? "等待工作流或紀錄產出",
